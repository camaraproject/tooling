...
**What you do:** on main, copy the reported template from `code/common/info-description-templates.yaml` into the affected API file's `info.description`, keeping the BEGIN/END delimiters and the copied text unchanged. For P-027, replace the whole drifted delimited block with the matching block from the local `code/common/info-description-templates.yaml` file — unless the message says the block is only missing the blank line directly after its `BEGIN` marker (a Commonalities r4.4+ requirement, needed so Swagger Editor renders the following Markdown heading); in that case just add the one blank line, the rest of the block is unaffected.
